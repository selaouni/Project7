...
def best_investment(data_list):
    ...
    la liste des action. Cette fonction donne comme résultat
    la solution optimisée en terme de profit et d'actions
    les plus rentables tout en respectant le montant total
    d'investissment de 500 euro.
    """
    start = time()
    matrice = [[0 for x in range(MAX_INVEST + 1)]
               for x in range(len(data_list)+1)]

    for i in range(1, len(data_list) + 1):
        for w in range(1, (MAX_INVEST + 1)):
            if data_list[i - 1][1] <= w:
                matrice[i][w] = max(data_list[i - 1][2] +
                                matrice[i - 1][int(w - data_list[i - 1][1])],
                                matrice[i - 1][w])

            else:
                # si max invest depassée prendre la solution
                # optimisée de la ligne d'avant
...
while x == True:
    ...
    try:
        for i in range(3):
            nbr = str(input("Saisir le num du dataset à analyser-1,2 ou 3 : "))
            csv_data = read_csv_file(nbr)
